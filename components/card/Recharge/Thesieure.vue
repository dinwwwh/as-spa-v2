<template>
  <GroupsSeparated v-if="isOpen">
    <div class="space-y-10">
      <HeadingsBase3>
        Nạp thẻ tự động 1
        <span class="text-base font-normal text-gray-500"> (2p/thẻ) </span>
      </HeadingsBase3>
...
export default {
  ...
  async fetch() {
    const { data: telcos } = await this.$axios.$get('thesieure/telcos')

    this.rootTelcos = telcos
  },
  computed: {
    isOpen() {
      return this.$app.findConfig('open_recharging_card_thesieure')
    },
    telcos() {
      if (!this.isOpen) return []
      return this.rootTelcos.map((telco) => telco.name)
    },
    faceValues() {
