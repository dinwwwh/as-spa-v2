<template>
  <GroupsSeparated>
    ...
    <div v-if="!$fetchState.pending" class="space-y-8">
      <div class="space-y-2">
        <HeadingsBase3> Cập nhật xác thực #{{ validatorId }} </HeadingsBase3>
        <MessagesInfo>
    ...
    <template v-if="!$fetchState.pending" #footer>
      <div class="flex items-center justify-end gap-3">
        <Buttons
          color="green"
          loading="completeUpdateValidator"
          @click="update"
        >
          Lưu
        </Buttons>
...
export default {
  ...
  async fetch() {
    const { data: validator } = await this.$axios.$get(
      `validators/${this.validatorId}`,
      {
        params: {
          _relationships: ['users'],
        },
      }
    )
  ...
  methods: {
    async update() {
      ...
      if (this.$v.validator.$invalid) {
        this.$nuxt.$emit('completeUpdateValidator')
