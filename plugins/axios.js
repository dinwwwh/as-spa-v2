export default function ({ $axios, redirect, error, $checkType, $notification }, inject) {
  ...
  $axios.onError(({ response }) => {
    ...
    if (response.status === 400) {
      $notification.error('Thông tin cung cấp không chính xác, hoặc hành động bị từ chối.')
      return;
    }
